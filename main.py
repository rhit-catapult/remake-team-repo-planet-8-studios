...
class Boss(pygame.sprite.Sprite):
    ...
    def draw_health_bar(self, surface):
        ...
        surface.blit(health_text, (SCREEN_WIDTH // 2 - health_text.get_width() // 2, 25))
...
def draw_start_menu(high_score):
    ...
    story_lines = [
        "In the distant future, after humans have conquered the solar system that we live in, they moved on to other solar systems, and eventually the entire universe."

        "They began to set their sights on planets further and further away from Earth, and found the planet Othryn."

        "On Othryn, Aliens led a peaceful life before the humans invaded. They were free, and could do whatever they wanted."
    ...
    for i, line in enumerate(story_lines):
        text = font_tiny.render(line, True, (200, 200, 255))
        screen.blit(text, (panel_rect.centerx - text.get_width() // 2, panel_rect.top + 100 + i * 50))

    # Draw controls panel
    ctrl_rect = pygame.Rect(SCREEN_WIDTH // 2 - 350, 570, 700, 250)
    ...
    for i, control in enumerate(controls):
        ...
        screen.blit(text, (ctrl_rect.centerx - text.get_width() // 2, ctrl_rect.top + 90 + i * 30))
...
platforms = [
    ...
    Platform(800, SCREEN_HEIGHT - 250, 200, 5),
    Platform(100, SCREEN_HEIGHT - 300, 200, 15),
    Platform(600, SCREEN_HEIGHT - 350, 150, 30),
...
while running:
    ...
    # Draw planet
    pygame.draw.circle(screen, PLANET_COLOR, (SCREEN_WIDTH - 150, 100), 80)
    pygame.draw.circle(screen, (PLANET_COLOR[0] - 20, PLANET_COLOR[1] - 20, PLANET_COLOR[2] - 20),
                       (SCREEN_WIDTH - 180, 70), 20)
    pygame.draw.circle(screen, (PLANET_COLOR[0] + 20, PLANET_COLOR[1] + 20, PLANET_COLOR[2] + 20),
                       (SCREEN_WIDTH - 120, 120), 30)
    ...
    if game_state == "start_menu":
        ...
        pygame.draw.rect(screen, (20, 10, 40, 180),
                         (SCREEN_WIDTH - 220, 5, 215, 80), border_radius=10)
        pygame.draw.rect(screen, (100, 70, 200),
                         (SCREEN_WIDTH - 220, 5, 215, 80), 2, border_radius=10)

        score_text = font_small.render(f"Score: {player.score}", True, WHITE)
        screen.blit(score_text, (SCREEN_WIDTH - score_text.get_width() - 15, 15))

        coins_text = font_small.render(f"Coins: {player.coins}", True, (255, 215, 0))
        screen.blit(coins_text, (SCREEN_WIDTH - coins_text.get_width() - 15, 50))

        # Draw upgrade status
        upgrades_text = font_tiny.render(f"Dmg: {player.damage_level:.1f}x | Spd: {player.speed_level:.1f}x", True,
                                         (150, 255, 150))
        screen.blit(upgrades_text, (SCREEN_WIDTH - upgrades_text.get_width() - 15, 80))
